...
<details>
<summary>참고: 초기 인증키 발급 절차 (완료됨)</summary>
...
2026-08-30, CI 보안 검사가 계속 실패했다. 원인 두 건 모두 Dependabot 자동 머지의 후유증이었다.

**① CodeQL — init 이 두 번 돌아 버전 불일치**

`codeql-action` 을 v4 로 올리면서 **v3 스텝을 지우지 않았다.**

```
init@v4  →  checkout 중복  →  init@v3  →  analyze@v4
                              ↑ 설정을 3.37.9 로 기록   ↑ 4.37.9 가 그걸 읽음 → 실패
```

`Loaded a configuration file for version '3.37.9', but running version '4.37.9'`

**② npm audit — next 내부의 취약한 postcss**

`next` 가 postcss 를 `8.4.31` 로 정확히 고정해 중첩 설치된다. 우리 직접 의존은 이미 패치된 `8.5.26` 이었지만 next 의 사본이 남아 있었다. `npm audit fix --force` 는 next 메이저 업그레이드를 요구하므로 `overrides` 로 `^8.5.26` 을 강제했다(같은 메이저 안의 마이너라 API 호환, 빌드 결과·번들 크기 동일 확인).
...
| **워크플로 파일** | **CI 로 잡히지 않는다.** 워크플로 자체가 바뀌므로 빌드는 통과하면서 워크플로만 깨진다. diff 를 직접 확인하고 머지 |
| 메이저 업그레이드 | 로컬에서 `npm run build` + 테스트 3종 확인 후 머지. `typescript@7` 로 `next build` 가 깨진 전례가 있다 |
...
- `next@16` 업그레이드 검토. 현재는 `overrides` 로 postcss 취약점만 막아둔 상태다 (8번 참고)
