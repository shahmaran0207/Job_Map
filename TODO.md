...
2026-09-05에 만든 "2인 교집합 통근권"이 수익화 첫 기능이다(무료로 계속 쓰기엔 이미
네이버부동산·직방이 앞서 있어서, 소비자가 돈 낼 이유를 만드는 게 목표 — 자세한 배경은
`docs/JOURNAL.md` 참고). 다음 단계:

- 후보지 여러 개(2인 교집합 포함) 나란히 비교 + PDF로 저장하는 일회성 결제 리포트
