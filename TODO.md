...
## 0. 다음에 할 것 (2026-09-08 기준)

우선순위 순. 각 항목에 필요한 작업이 무엇인지 적혀 있다.

### ① 유료 후보지 비교 리포트로 패키징 ← 지금 당장

"2인 교집합 통근권"(2026-09-05)과 후보지 저장/불러오기(`app/lib/candidates.ts`,
2026-09-08, `localStorage` 기반, 결제 없음)까지 만들었다 — 이게 수익화 첫 기능이다
(무료로 계속 쓰기엔 이미 네이버부동산·직방이 앞서 있어서, 소비자가 돈 낼 이유를
만드는 게 목표. 자세한 배경은 `docs/JOURNAL.md` 참고). 다음 단계:

- 저장한 후보지 여러 개를 나란히 비교하는 화면(지금은 사이드바에 카드 리스트만 있음)
...
- `feat/saved-candidates` 브랜치 → `main` 병합 필요
...
## 4. 라우팅 엔진 — 도보·자차 정상 동작 (2026-09-08)

`npm run routing:up` 으로 엔진 기동. `npm run routing:check` 로 확인:

```
✓ walk     폴리곤 1개 / 61점    (0.0~0.1초)
✓ drive    폴리곤 1개 / 2836점  (0.7~0.9초)
✗ transit  타임아웃 — 정상(OTP2 + GTFS 미구성)
```
