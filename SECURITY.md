...
## 2. 위협: 라우팅 API 키 유출 / 대중교통 그래프 남용

**배경 (2026-09-15)** — 원래 Valhalla/OTP2를 개발 PC Docker에 띄우고 Cloudflare Tunnel로 노출하는 계획이었다. "PC 상시 가동"이 배포 전제와 맞지 않아 폐기하고, 자체 서버가 아예 없는 구조(도보/자차=OpenRouteService 공개 API, 대중교통=minotor를 Vercel 함수 안에서 직접 실행)로 바꿨다. 그러면서 무인증 엔진 노출이라는 예전 최우선 위협 자체가 사라졌다. 남은 위협은 두 가지뿐이다.

**시나리오 1 — ORS_API_KEY 유출**: 클라이언트로 내려가면 무료 할당량(일 2,500건)을 제3자가 소진시킬 수 있다.

**방어**: `ORS_API_KEY`는 `app/api/rents` 서버 사이드에서만 쓰고 어떤 API 응답에도 포함하지 않는다. 프론트엔드 번들에 노출되는 키는 `NEXT_PUBLIC_` 접두사가 없는 한 없다 — 리뷰에서 이 접두사가 라우팅 관련 키에 붙으면 반려한다.

**시나리오 2 — 대중교통 등시선 남용**: minotor는 서버 프로세스 안에서 도는 순수 계산이라 요청량이 곧 CPU/메모리 비용이다(Vercel 함수 실행시간 과금).

**방어**:
- `rateLimit()` (`src/lib/rate-limit.ts`) 이 `/api/rents` 전체에 이미 걸려 있다
- 애플리케이션 측에서도 시간 값을 열거형으로 제한한다 (`assertAllowedMinutes`) — 임의 값으로 초대형 등시선을 요청하는 것을 막는다
- 좌표는 대한민국 경계 검사를 통과해야 계산에 들어간다 (`assertKoreanCoord`)
- 등시선 결과는 `isochrone_cache`에 영구 캐시된다 — 같은 격자·모드·시간대의 반복 요청은 계산 자체를 건너뛴다
